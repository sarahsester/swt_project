...
class LRUCache(object):
    ...
    # scale.  But as long as it's only used as storage for templates_old this
    # won't do any harm.
...
def select_autoescape(
    ...
    If you want to enable it for all templates_old created from strings or
    for all templates_old with `.html` and `.xml` extensions::
    ...
    a list of all templates_old it should be disabled for.  If a template is
...
def htmlsafe_json_dumps(obj, dumper=None, **kwargs):
    ...
    this is available in templates_old through the ``|tojson`` filter which will
...
class Cycler(object):
    ...
    once the end is reached. Available as ``cycler`` in templates_old.
...
class Joiner(object):
    """A joining helper for templates_old."""
